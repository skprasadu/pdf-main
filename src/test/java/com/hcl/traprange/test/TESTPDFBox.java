...
public class TESTPDFBox extends PDFTextStripper {
	...
	@Test
	public void testExtractWordLocations() throws IOException, ParseException {
		extractContent("sample-13.pdf");
	}
	...
	private int[] buildIndex(PDDocument doc) throws IOException {
		...
		PDFTextStripper stripper = new PDFTextStripper() {
			...
			protected void writeString(String text, List<TextPosition> textPositions) throws IOException {
				...
				int lineNum = line.getAndIncrement();
				if(text.trim().equals(endTable)){
				...
				if(text.trim().equals(startTable)){
					skipLine.set(true);
					return;
				}
			}
		...
		int []sl  = new int[skipLines.size()];
		int idx = 0;
		for(Integer in: skipLines){
			sl[idx++] = in;
		}
		return sl;
	}
